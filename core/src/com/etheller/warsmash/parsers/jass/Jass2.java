...
public class Jass2 {
	...
	public static final class CommonEnvironment {
		...
		private CommonEnvironment(final JassProgram jassProgramVisitor, final DataSource dataSource,
			...
			// IssuePointOrder: string-name version of IssuePointOrderById
			jassProgramVisitor.getJassNativeManager().createNative("IssuePointOrder",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = arguments.get(0).visit(ObjectJassValueVisitor.getInstance());
						if (whichUnit == null) {
							return BooleanJassValue.FALSE;
			...
			jassProgramVisitor.getJassNativeManager().createNative("CountUnitsInGroup",
					(arguments, globalScope, triggerScope) -> {
						final List<CUnit> group = nullable(arguments, 0,
								ObjectJassValueVisitor.<List<CUnit>>getInstance());
						return IntegerJassValue.of(group != null ? group.size() : 0);
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("ChooseRandomNPBuilding",
					(arguments, globalScope, triggerScope) -> {
						// Random NP-building selection is not implemented; return 0
						return IntegerJassValue.of(0);
					});
			// ============================================================================
			// Force API
			...
			jassProgramVisitor.getJassNativeManager().createNative("GetCameraMargin",
					(arguments, globalScope, triggerScope) -> {
						...
						default:
							System.err.println("GetCameraMargin: invalid margin index: " + whichMargin);
							return RealJassValue.ZERO;
						}
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("GetFloatGameState",
					(arguments, globalScope, triggerScope) -> {
						...
						default:
							System.err.println("GetFloatGameState: unhandled state: " + gameState);
							return RealJassValue.ZERO;
						}
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("SetFloatGameState",
					(arguments, globalScope, triggerScope) -> {
						...
						case TIME_OF_DAY:
							...
							return null;
						default:
							System.err.println("SetFloatGameState: unhandled state: " + gameState);
							return null;
						}
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("KillSoundWhenDone",
					(arguments, globalScope, triggerScope) -> {
						// sounds already stop naturally when done in our implementation
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetSoundIsPlaying",
					(arguments, globalScope, triggerScope) -> {
						final CSound soundHandle = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						if (soundHandle == null) {
							return BooleanJassValue.FALSE;
						}
						return BooleanJassValue.of(soundHandle.isPlaying());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetSoundIsLoading",
					(arguments, globalScope, triggerScope) -> {
						// sounds are loaded synchronously; never in a loading state
						return BooleanJassValue.FALSE;
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetSoundFileDuration",
					(arguments, globalScope, triggerScope) -> {
						final String soundFile = nullable(arguments, 0, StringJassValueVisitor.getInstance());
						if (soundFile == null) {
							return IntegerJassValue.of(0);
						}
						try {
							final Sound s = UnitSound.createSound(war3MapViewer.mapMpq, soundFile);
							if (s != null) {
								return IntegerJassValue
										.of((int) (com.etheller.warsmash.viewer5.gl.Extensions.audio.getDuration(s)
												* 1000));
							}
						}
			...
			jassProgramVisitor.getJassNativeManager().createNative("SetSoundPosition",
					(arguments, globalScope, triggerScope) -> {
						final CSound soundHandle = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final float x = arguments.get(1).visit(RealJassValueVisitor.getInstance()).floatValue();
						final float y = arguments.get(2).visit(RealJassValueVisitor.getInstance()).floatValue();
			...
			jassProgramVisitor.getJassNativeManager().createNative("AttachSoundToUnit",
					(arguments, globalScope, triggerScope) -> {
						final CSound soundHandle = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final CUnit whichUnit = nullable(arguments, 1, ObjectJassValueVisitor.getInstance());
						if ((soundHandle != null) && (whichUnit != null)) {
							soundHandle.setPosition(whichUnit.getX(), whichUnit.getY(), 0);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("SetSoundDistanceCutoff",
					(arguments, globalScope, triggerScope) -> {
						final CSound soundHandle = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						if (soundHandle instanceof CSoundFilename) {
							final float cutoff = arguments.get(1).visit(RealJassValueVisitor.getInstance())
									.floatValue();
							((CSoundFilename) soundHandle).setDistanceCutoff(cutoff);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("SetSoundChannel",
					(arguments, globalScope, triggerScope) -> {
						// channel routing to audio hardware is not implemented; accepted as no-op
						return null;
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("SetUnitFacingTimed",
					(arguments, globalScope, triggerScope) -> {
						// time-interpolated rotation is not yet simulated; apply facing immediately
						final CUnit whichUnit = arguments.get(0).visit(ObjectJassValueVisitor.getInstance());
						final double facing = arguments.get(1).visit(RealJassValueVisitor.getInstance());
						whichUnit.setFacing((float) facing);
						return null;
			...
			jassProgramVisitor.getJassNativeManager().createNative("GetHeroXP",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						if (whichUnit == null) {
							return IntegerJassValue.of(0);
						...
						final CAbilityHero heroData = whichUnit.getHeroData();
						if (heroData != null) {
							return IntegerJassValue.of(heroData.getXp());
						}
						return IntegerJassValue.of(0);
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("GetUnitLevel",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						if (whichUnit == null) {
							return IntegerJassValue.of(0);
			...
			jassProgramVisitor.getJassNativeManager().createNative("GetHeroSkillPoints",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						if (whichUnit == null) {
							return IntegerJassValue.of(0);
						...
						final CAbilityHero heroData = whichUnit.getHeroData();
						if (heroData != null) {
							return IntegerJassValue.of(heroData.getSkillPoints());
						}
						return IntegerJassValue.of(0);
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("GetHeroStr",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean includeBonuses = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						if (whichUnit == null) {
							return IntegerJassValue.of(0);
						}
						final CAbilityHero heroData = whichUnit.getHeroData();
						if (heroData != null) {
							return IntegerJassValue.of(includeBonuses
									? heroData.getStrength().getCurrent()
									: heroData.getStrength().getCurrentBase());
						}
						return IntegerJassValue.of(0);
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetHeroAgi",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean includeBonuses = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						if (whichUnit == null) {
						...
						if (heroData != null) {
							return IntegerJassValue.of(includeBonuses
									? heroData.getAgility().getCurrent()
									: heroData.getAgility().getCurrentBase());
						}
						return IntegerJassValue.of(0);
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetHeroInt",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean includeBonuses = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						if (whichUnit == null) {
						...
						if (heroData != null) {
							return IntegerJassValue.of(includeBonuses
									? heroData.getIntelligence().getCurrent()
									: heroData.getIntelligence().getCurrentBase());
						}
						return IntegerJassValue.of(0);
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetHeroProperName",
					(arguments, globalScope, triggerScope) -> {
						...
						final CAbilityHero heroData = whichUnit.getHeroData();
						if (heroData != null) {
							final String properName = heroData.getProperName();
			...
			jassProgramVisitor.getJassNativeManager().createNative("UnitAddHeroSkill",
					(arguments, globalScope, triggerScope) -> {
						// Directly adding a hero skill requires learnSkill infrastructure;
						// award a skill point and let normal UI handle it for now
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						if (whichUnit != null) {
							final CAbilityHero heroData = whichUnit.getHeroData();
							if (heroData != null) {
								heroData.setSkillPoints(heroData.getSkillPoints() + 1);
							}
						}
						return BooleanJassValue.FALSE;
					});
			jassProgramVisitor.getJassNativeManager().createNative("SetHeroSkillAvailable",
					(arguments, globalScope, triggerScope) -> {
			...
			jassProgramVisitor.getJassNativeManager().createNative("DestroyTimerDialog",
					(arguments, globalScope, triggerScope) -> {
			...
			jassProgramVisitor.getJassNativeManager().createNative("TimerDialogSetTitle",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.ui.dialog.CTimerDialog td = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final String title = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						if (td != null) {
							td.setTitle(CommonEnvironment.this.gameUI, this.gameUI.getTrigStr(title));
						}
						return null;
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("SetSoundParamsFromLabel",
					(arguments, globalScope, triggerScope) -> {
						// sound params from label are resolved at create time; no runtime effect
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("SetSoundDuration",
					(arguments, globalScope, triggerScope) -> {
						// duration override not supported at runtime; ignored
						return null;
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("TriggerRegisterAnyUnitEventBJ",
					(arguments, globalScope, triggerScope) -> {
						final Trigger whichTrigger = arguments.get(0).visit(ObjectJassValueVisitor.getInstance());
						final JassGameEventsWar3 whichEvent = arguments.get(1)
								.visit(ObjectJassValueVisitor.getInstance());
						return new HandleJassValue(eventType,
								CommonEnvironment.this.simulation.registerGlobalUnitEvent(whichTrigger, whichEvent,
										null));
					});
			...
			// Campaign parity batch: retail campaign scripts reach these natives
			// (see docs/CAMPAIGN_NATIVE_COVERAGE.md). Doodad/destructable visuals,
			// minimap icons and sound distances have no simulation yet, so they are
			// accepted and ignored; gameplay queries use real unit state where
			// available. Shop stock is tracked per shop (item catalog + counts with
			// out-of-stock purchase gating); unit stock counts are recorded.
			// Player-state events fire on the rising edge via CPlayerStateEvent.
			jassProgramVisitor.getJassNativeManager().createNative("AddItemToStock",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final int itemId = arguments.get(1).visit(IntegerJassValueVisitor.getInstance());
						...
						if (whichUnit != null) {
							CAbilitySellItems sellItems = null;
							for (final CAbility ability : whichUnit.getAbilities()) {
								if (ability instanceof CAbilitySellItems) {
									sellItems = (CAbilitySellItems) ability;
									break;
								}
							}
							if (sellItems == null) {
								sellItems = new CAbilitySellItems(
										CommonEnvironment.this.simulation.getHandleIdAllocator().createId(),
										new ArrayList<War3ID>());
								whichUnit.add(CommonEnvironment.this.simulation, sellItems);
							}
							sellItems.addItemToStock(new War3ID(itemId), currentStock, stockMax);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("RemoveItemFromStock",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final int itemId = arguments.get(1).visit(IntegerJassValueVisitor.getInstance());
						if (whichUnit != null) {
							for (final CAbility ability : whichUnit.getAbilities()) {
								if (ability instanceof CAbilitySellItems) {
									((CAbilitySellItems) ability).removeItemFromStock(new War3ID(itemId));
									break;
								}
							}
						}
			...
			jassProgramVisitor.getJassNativeManager().createNative("AddUnitToStock",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final int unitId = arguments.get(1).visit(IntegerJassValueVisitor.getInstance());
						final int currentStock = arguments.get(2).visit(IntegerJassValueVisitor.getInstance());
			...
			jassProgramVisitor.getJassNativeManager().createNative("RemoveUnitFromStock",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final int unitId = arguments.get(1).visit(IntegerJassValueVisitor.getInstance());
						if (whichUnit != null) {
							whichUnit.removeUnitFromStock(new War3ID(unitId));
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("SetItemTypeSlots",
					(arguments, globalScope, triggerScope) -> null);
			jassProgramVisitor.getJassNativeManager().createNative("SetUnitTypeSlots",
					(arguments, globalScope, triggerScope) -> null);
			...
			jassProgramVisitor.getJassNativeManager().createNative("RestartGame",
					(arguments, globalScope, triggerScope) -> {
						...
						// through the ChangeLevel path with the running map so the
						// menu reloads it the same way.
						final boolean doScoreScreen = !arguments.isEmpty()
								&& arguments.get(0).visit(BooleanJassValueVisitor.getInstance());
						final String currentMapPath = war3MapViewer.getCurrentMapPath();
			...
			jassProgramVisitor.getJassNativeManager().createNative("UnitRemoveBuffs",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						if (whichUnit != null) {
							for (final CAbility ability : new ArrayList<>(whichUnit.getAbilities())) {
								if (ability instanceof CBuff) {
									whichUnit.remove(CommonEnvironment.this.simulation, (CBuff) ability);
								}
							}
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("UnitRemoveBuffsEx",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						// Filter flags (positive/negative, magic/physical, timedLife,
						// aura, autoDispel) are not yet modeled; clear all buffs.
						if (whichUnit != null) {
							for (final CAbility ability : new ArrayList<>(whichUnit.getAbilities())) {
								if (ability instanceof CBuff) {
									whichUnit.remove(CommonEnvironment.this.simulation, (CBuff) ability);
			...
			jassProgramVisitor.getJassNativeManager().createNative("TriggerRegisterPlayerStateEvent",
					(arguments, globalScope, triggerScope) -> {
						final Trigger trigger = arguments.get(0).visit(ObjectJassValueVisitor.<Trigger>getInstance());
						final CPlayer whichPlayer = nullable(arguments, 1, ObjectJassValueVisitor.getInstance());
						final CPlayerState whichState = nullable(arguments, 2,
								ObjectJassValueVisitor.getInstance());
						final CLimitOp opcode = nullable(arguments, 3, ObjectJassValueVisitor.getInstance());
						final double limitval = arguments.get(4).visit(RealJassValueVisitor.getInstance());
						if ((whichPlayer == null) || (whichState == null) || (opcode == null)) {
							return eventType.getNullValue();
						}
						return new HandleJassValue(eventType,
								CommonEnvironment.this.simulation.registerPlayerStateEvent(globalScope, trigger,
										whichPlayer, whichState, opcode, limitval));
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetEventPlayerState",
					(arguments, globalScope, triggerScope) -> {
						if (triggerScope instanceof CommonTriggerExecutionScope) {
							final CPlayerState state = ((CommonTriggerExecutionScope) triggerScope)
									.getEventPlayerState();
							if (state != null) {
								return new HandleJassValue(playerstateType, state);
							}
						}
			...
			jassProgramVisitor.getJassNativeManager().createNative("SetDoodadAnimationRect",
					(arguments, globalScope, triggerScope) -> null);
			jassProgramVisitor.getJassNativeManager().createNative("UnitWakeUp",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						if (whichUnit != null) {
							whichUnit.removeUnitType(CommonEnvironment.this.simulation, CUnitTypeJass.SLEEPING);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("UnitAddSleep",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean add = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						if (whichUnit != null) {
							if (add) {
								whichUnit.addUnitType(CommonEnvironment.this.simulation, CUnitTypeJass.SLEEPING);
							}
							else {
								whichUnit.removeUnitType(CommonEnvironment.this.simulation, CUnitTypeJass.SLEEPING);
							}
			...
			jassProgramVisitor.getJassNativeManager().createNative("UnitIsSleeping",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						return BooleanJassValue
								.of((whichUnit != null) && whichUnit.isUnitType(CUnitTypeJass.SLEEPING));
					});
			jassProgramVisitor.getJassNativeManager().createNative("UnitIgnoreAlarm",
					(arguments, globalScope, triggerScope) -> {
						// No alarm state is simulated; echo the request so scripts
						// observe the flag they just set.
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean flag = arguments.size() > 1
								&& arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						return BooleanJassValue.of((whichUnit != null) && flag);
					});
			jassProgramVisitor.getJassNativeManager().createNative("SetAltMinimapIcon",
					(arguments, globalScope, triggerScope) -> null);
			jassProgramVisitor.getJassNativeManager().createNative("UnitSetUsesAltIcon",
					(arguments, globalScope, triggerScope) -> null);
			jassProgramVisitor.getJassNativeManager().createNative("UnitResetCooldown",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						if (whichUnit != null) {
							whichUnit.clearAllAbilityCooldowns();
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("SetDestructableAnimationSpeed",
					(arguments, globalScope, triggerScope) -> null);
			jassProgramVisitor.getJassNativeManager().createNative("QueueDestructableAnimation",
					(arguments, globalScope, triggerScope) -> null);
			...
			jassProgramVisitor.getJassNativeManager().createNative("SetSoundDistances",
					(arguments, globalScope, triggerScope) -> {
						final CSound soundHandle = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final float maxDist = arguments.size() > 2
								? arguments.get(2).visit(RealJassValueVisitor.getInstance()).floatValue()
								: 99999f;
						if (soundHandle instanceof CSoundFilename) {
							((CSoundFilename) soundHandle).setDistanceCutoff(maxDist);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetEventDamage",
					(arguments, globalScope, triggerScope) -> RealJassValue.ZERO);
			jassProgramVisitor.getJassNativeManager().createNative("IsUnitIdType",
					(arguments, globalScope, triggerScope) -> {
						final int unitId = arguments.get(0).visit(IntegerJassValueVisitor.getInstance());
						final CUnitTypeJass whichUnitType = nullable(arguments, 1,
								ObjectJassValueVisitor.getInstance());
						final CUnitType type = CommonEnvironment.this.simulation.getUnitData()
								.getUnitType(new War3ID(unitId));
						if ((type == null) || (whichUnitType == null)) {
							return BooleanJassValue.FALSE;
						}
						switch (whichUnitType) {
						case HERO:
							return BooleanJassValue.of(type.isHero());
						case STRUCTURE:
							return BooleanJassValue.of(type.isBuilding());
						case FLYING:
							return BooleanJassValue.of(type.getTargetedAs().contains(CTargetType.AIR));
						case GROUND:
							return BooleanJassValue.of(type.getTargetedAs().contains(CTargetType.GROUND));
						case GIANT:
							return BooleanJassValue.of(type.getClassifications().contains(
									CUnitClassification.GIANT));
						case SUMMONED:
							return BooleanJassValue.of(type.getClassifications().contains(
									CUnitClassification.SUMMONED));
						case UNDEAD:
							return BooleanJassValue.of(type.getClassifications().contains(
									CUnitClassification.UNDEAD));
						case MECHANICAL:
							return BooleanJassValue.of(type.getClassifications().contains(
									CUnitClassification.MECHANICAL));
						case PEON:
							return BooleanJassValue.of(type.getClassifications().contains(
									CUnitClassification.PEON));
						case SAPPER:
							return BooleanJassValue.of(type.getClassifications().contains(
									CUnitClassification.SAPPER));
						case TOWNHALL:
							return BooleanJassValue.of(type.getClassifications().contains(
									CUnitClassification.TOWNHALL));
						case ANCIENT:
							return BooleanJassValue.of(type.getClassifications().contains(
									CUnitClassification.ANCIENT));
						case TAUREN:
							return BooleanJassValue.of(type.getClassifications().contains(
									CUnitClassification.TAUREN));
						default:
							return BooleanJassValue.FALSE;
						}
					});
			jassProgramVisitor.getJassNativeManager().createNative("UnitApplyTimedLife",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final int buffId = arguments.get(1).visit(IntegerJassValueVisitor.getInstance());
						final float duration = arguments.get(2).visit(RealJassValueVisitor.getInstance())
								.floatValue();
						if (whichUnit != null) {
							whichUnit.add(CommonEnvironment.this.simulation,
									new CBuffTimedLife(
											CommonEnvironment.this.simulation.getHandleIdAllocator().createId(),
											new War3ID(buffId), duration, false));
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("IsUnitInTransport",
					(arguments, globalScope, triggerScope) -> BooleanJassValue.FALSE);
			jassProgramVisitor.getJassNativeManager().createNative("IsUnitInvisible",
					(arguments, globalScope, triggerScope) -> BooleanJassValue.FALSE);
			jassProgramVisitor.getJassNativeManager().createNative("IsUnitLoaded",
					(arguments, globalScope, triggerScope) -> BooleanJassValue.FALSE);
			jassProgramVisitor.getJassNativeManager().createNative("SetBlightRect",
					(arguments, globalScope, triggerScope) -> {
						final Rectangle rect = nullable(arguments, 1, ObjectJassValueVisitor.getInstance());
						final boolean addBlight = arguments.size() > 2
								&& arguments.get(2).visit(BooleanJassValueVisitor.getInstance());
						if (rect != null) {
							final float centerX = rect.x + (rect.width / 2);
							final float centerY = rect.y + (rect.height / 2);
							final float radius = Math.max(rect.width, rect.height) / 2;
							war3MapViewer.setBlight(centerX, centerY, radius, addBlight);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("TerrainDeformCrater",
					(arguments, globalScope, triggerScope) -> new HandleJassValue(terraindeformationType, null));
			jassProgramVisitor.getJassNativeManager().createNative("UnitRemoveItemFromSlot",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						...
						final CItem itemInSlot = inventoryData.getItemInSlot(whichSlot);
						if (itemInSlot == null) {
							return new HandleJassValue(itemType, null);
						}
						inventoryData.dropItem(CommonEnvironment.this.simulation, whichUnit, itemInSlot,
								whichUnit.getX(), whichUnit.getY(), true);
						return new HandleJassValue(itemType, itemInSlot);
					});
			jassProgramVisitor.getJassNativeManager().createNative("UnitUseItemPoint",
					(arguments, globalScope, triggerScope) -> BooleanJassValue.FALSE);
			...
			jassProgramVisitor.getJassNativeManager().createNative("QuestSetTitle",
					(arguments, globalScope, triggerScope) -> {
						...
						final String title = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						if (quest != null) {
							quest.setTitle(this.gameUI.getTrigStr(title));
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("QuestSetDescription",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final String desc = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						if (quest != null) {
							quest.setDescription(this.gameUI.getTrigStr(desc));
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("QuestSetIconPath",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final String path = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						if (quest != null) {
							quest.setIconPath(path != null ? path : "");
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("QuestSetEnabled",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
			...
			jassProgramVisitor.getJassNativeManager().createNative("QuestSetRequired",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean required = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						if (quest != null) {
							quest.setRequired(required);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("QuestSetCompleted",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean completed = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						if (quest != null) {
							quest.setCompleted(completed);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("QuestSetFailed",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean failed = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						if (quest != null) {
							quest.setFailed(failed);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("QuestSetDiscovered",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean discovered = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						if (quest != null) {
							quest.setDiscovered(discovered);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("IsQuestEnabled",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return BooleanJassValue.of((quest != null) && quest.isEnabled());
					});
			jassProgramVisitor.getJassNativeManager().createNative("IsQuestRequired",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return BooleanJassValue.of((quest != null) && quest.isRequired());
					});
			jassProgramVisitor.getJassNativeManager().createNative("IsQuestCompleted",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return BooleanJassValue.of((quest != null) && quest.isCompleted());
					});
			jassProgramVisitor.getJassNativeManager().createNative("IsQuestFailed",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return BooleanJassValue.of((quest != null) && quest.isFailed());
					});
			jassProgramVisitor.getJassNativeManager().createNative("IsQuestDiscovered",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return BooleanJassValue.of((quest != null) && quest.isDiscovered());
					});
			jassProgramVisitor.getJassNativeManager().createNative("CreateQuestItem",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuest quest = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						if (quest == null) {
						...
						final String desc = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						return new HandleJassValue(questitemType,
								quest.createItem(this.gameUI.getTrigStr(desc)));
					});
			jassProgramVisitor.getJassNativeManager().createNative("DestroyQuestItem",
					(arguments, globalScope, triggerScope) -> {
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("QuestItemSetDescription",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuestItem item = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final String desc = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						if (item != null) {
							item.setDescription(this.gameUI.getTrigStr(desc));
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("QuestItemSetCompleted",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuestItem item = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean completed = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
			...
			jassProgramVisitor.getJassNativeManager().createNative("IsQuestItemCompleted",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.quest.CQuestItem item = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return BooleanJassValue.of((item != null) && item.isCompleted());
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("IsMultiboardDisplayed",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboard mb = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return BooleanJassValue.of((mb != null) && mb.isDisplayed());
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardMinimize",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboard mb = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean minimize = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						if (mb != null) {
							mb.setMinimized(minimize);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("IsMultiboardMinimized",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboard mb = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return BooleanJassValue.of((mb != null) && mb.isMinimized());
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSetTitleText",
					(arguments, globalScope, triggerScope) -> {
						...
						if (mb != null) {
							mb.setTitle(this.gameUI.getTrigStr(text));
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardGetTitleText",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboard mb = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return new StringJassValue(mb != null ? mb.getTitle() : "");
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSetTitleTextColor",
					(arguments, globalScope, triggerScope) -> {
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardGetRowCount",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboard mb = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return IntegerJassValue.of(mb != null ? mb.getRows() : 0);
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardGetColumnCount",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboard mb = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						return IntegerJassValue.of(mb != null ? mb.getColumns() : 0);
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSetColumnCount",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboard mb = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final int count = arguments.get(1).visit(IntegerJassValueVisitor.getInstance());
						if (mb != null) {
							mb.setColumns(count);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSetRowCount",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboard mb = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final int count = arguments.get(1).visit(IntegerJassValueVisitor.getInstance());
			...
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardGetItem",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboard mb = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						if (mb == null) {
							return multiboarditemType.getNullValue();
						}
						final int row = arguments.get(1).visit(IntegerJassValueVisitor.getInstance());
						final int col = arguments.get(2).visit(IntegerJassValueVisitor.getInstance());
						return new HandleJassValue(multiboarditemType, mb.getItem(row, col));
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardReleaseItem",
					(arguments, globalScope, triggerScope) -> {
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSetItemStyle",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboardItem item = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final boolean showValue = arguments.get(1).visit(BooleanJassValueVisitor.getInstance());
						final boolean showIcon = arguments.get(2).visit(BooleanJassValueVisitor.getInstance());
						if (item != null) {
							item.setStyle((showValue ? 1 : 0) | (showIcon ? 2 : 0));
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSetItemValue",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboardItem item = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final String val = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						if (item != null) {
							item.setValue(this.gameUI.getTrigStr(val));
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSetItemValueColor",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboardItem item = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final float r = arguments.get(1).visit(IntegerJassValueVisitor.getInstance()) / 255.0f;
						final float g = arguments.get(2).visit(IntegerJassValueVisitor.getInstance()) / 255.0f;
						final float b = arguments.get(3).visit(IntegerJassValueVisitor.getInstance()) / 255.0f;
						final float a = arguments.get(4).visit(IntegerJassValueVisitor.getInstance()) / 255.0f;
						if (item != null) {
							item.setValueColor(r, g, b, a);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSetItemWidth",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboardItem item = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final float width = arguments.get(1).visit(RealJassValueVisitor.getInstance()).floatValue();
			...
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSetItemIcon",
					(arguments, globalScope, triggerScope) -> {
						final com.etheller.warsmash.viewer5.handlers.w3x.simulation.ui.CMultiboardItem item = nullable(
								arguments, 0, ObjectJassValueVisitor.getInstance());
						final String icon = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						if (item != null) {
							item.setIcon(icon != null ? icon : "");
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("MultiboardSuppressDisplay",
					(arguments, globalScope, triggerScope) -> {
			...
			jassProgramVisitor.getJassNativeManager().createNative("PlayCinematic",
					(arguments, globalScope, triggerScope) -> {
						...
						if (currentThread != null) {
							...
							// Real movies sleep for their decoded duration;
							// the overlay fallback keeps the historic ~5s.
							final float movieDuration = meleeUI.getMovieDurationSeconds();
							final CTimerSleepAction timer = new CTimerSleepAction(currentThread);
							timer.setRepeats(false);
							timer.setTimeoutTime(movieDuration > 0 ? movieDuration : 5.0f);
							meleeUI.bindMovieSleepTimer(timer);
							timer.start(CommonEnvironment.this.simulation);
						}
						return null;
					});
			...
			// behaviour is visible even without full cinematic queue support.
			jassProgramVisitor.getJassNativeManager().createNative("TransmissionFromUnit",
					(arguments, globalScope, triggerScope) -> {
						final CUnit whichUnit = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final CPlayerColor color = nullable(arguments, 1, ObjectJassValueVisitor.getInstance());
			...
			// Hero event getters
			jassProgramVisitor.getJassNativeManager().createNative("GetLevelingUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getLevelingUnit());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetLearningUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getLearningUnit());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetLearnedSkill",
					(arguments, globalScope, triggerScope) -> {
						return IntegerJassValue.of(((CommonTriggerExecutionScope) triggerScope).getLearnedSkill());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetLearnedSkillBJ",
					(arguments, globalScope, triggerScope) -> {
						return IntegerJassValue.of(((CommonTriggerExecutionScope) triggerScope).getLearnedSkill());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetLearnedSkillLevel",
					(arguments, globalScope, triggerScope) -> {
						return IntegerJassValue
								.of(((CommonTriggerExecutionScope) triggerScope).getLearnedSkillLevel());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetRevivingUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getRevivingUnit());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetRevivableUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getRevivableUnit());
					});
			// Unit lifecycle event getters
			jassProgramVisitor.getJassNativeManager().createNative("GetDecayingUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getDecayingUnit());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetCancelledStructure",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getCancelledStructure());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetConstructingStructure",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getConstructingStructure());
					});
			// Summon event getters
			jassProgramVisitor.getJassNativeManager().createNative("GetSummonedUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getSummonedUnit());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetSummoningUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getSummoningUnit());
					});
			// Transport event getters
			jassProgramVisitor.getJassNativeManager().createNative("GetLoadedUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getLoadedUnit());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetTransportUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getTransportUnit());
					});
			// Detect/sell event getters
			jassProgramVisitor.getJassNativeManager().createNative("GetDetectedUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getDetectedUnit());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetSellingUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getSellingUnit());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetSoldUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getSoldUnit());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetBuyingUnit",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getBuyingUnit());
					});
			...
			jassProgramVisitor.getJassNativeManager().createNative("GetSpellTargetItem",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(itemType,
								((CommonTriggerExecutionScope) triggerScope).getSpellTargetItem());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetSpellTargetDestructable",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(destructableType,
								((CommonTriggerExecutionScope) triggerScope).getSpellTargetDestructable());
					});
			// Rescue event getter
			jassProgramVisitor.getJassNativeManager().createNative("GetRescuer",
					(arguments, globalScope, triggerScope) -> {
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getRescuer());
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetRescuedUnit",
					(arguments, globalScope, triggerScope) -> {
						// rescued unit is the same as the triggering unit
						return new HandleJassValue(unitType,
								((CommonTriggerExecutionScope) triggerScope).getTriggeringUnit());
					});
			...
			// Patch 1.23+ crap - gamecache natives
			// Gamecache is used for campaign hero carry-over between maps
			...
			jassProgramVisitor.getJassNativeManager().createNative("StoreString",
					(arguments, globalScope, triggerScope) -> {
						...
						if (cache != null && missionKey != null && key != null && value != null) {
							cache.storeString(missionKey, key, value);
							return BooleanJassValue.TRUE;
						}
						return BooleanJassValue.FALSE;
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetStoredInteger",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							return IntegerJassValue.of(cache.getStoredInteger(missionKey, key));
						}
						return IntegerJassValue.of(0);
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetStoredReal",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							return RealJassValue.of(cache.getStoredReal(missionKey, key));
						}
						return RealJassValue.ZERO;
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetStoredBoolean",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							return BooleanJassValue.of(cache.getStoredBoolean(missionKey, key));
						}
						return BooleanJassValue.FALSE;
					});
			jassProgramVisitor.getJassNativeManager().createNative("GetStoredString",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							return StringJassValue.of(cache.getStoredString(missionKey, key));
						}
						return StringJassValue.of("");
					});
			jassProgramVisitor.getJassNativeManager().createNative("HaveStoredInteger",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							return BooleanJassValue.of(cache.haveStoredInteger(missionKey, key));
						}
						return BooleanJassValue.FALSE;
					});
			jassProgramVisitor.getJassNativeManager().createNative("HaveStoredReal",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							return BooleanJassValue.of(cache.haveStoredReal(missionKey, key));
						}
						return BooleanJassValue.FALSE;
					});
			jassProgramVisitor.getJassNativeManager().createNative("HaveStoredBoolean",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							return BooleanJassValue.of(cache.haveStoredBoolean(missionKey, key));
						}
						return BooleanJassValue.FALSE;
					});
			jassProgramVisitor.getJassNativeManager().createNative("HaveStoredString",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							return BooleanJassValue.of(cache.haveStoredString(missionKey, key));
						}
						return BooleanJassValue.FALSE;
					});
			jassProgramVisitor.getJassNativeManager().createNative("FlushStoredInteger",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							cache.flushStoredInteger(missionKey, key);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("FlushStoredReal",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							cache.flushStoredReal(missionKey, key);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("FlushStoredBoolean",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							cache.flushStoredBoolean(missionKey, key);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("FlushStoredString",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
						final String key = nullable(arguments, 2, StringJassValueVisitor.getInstance());
						if (cache != null && missionKey != null && key != null) {
							cache.flushStoredString(missionKey, key);
						}
						return null;
					});
			jassProgramVisitor.getJassNativeManager().createNative("FlushStoredMission",
					(arguments, globalScope, triggerScope) -> {
						final CGameCache cache = nullable(arguments, 0, ObjectJassValueVisitor.getInstance());
						final String missionKey = nullable(arguments, 1, StringJassValueVisitor.getInstance());
			...
			jassProgramVisitor.getJassNativeManager().createNative("SetOrderButtonHidden",
					(arguments, globalScope, triggerScope) -> {
						...
						// SetOrderButtonHidden not yet fully implemented
						System.err.println("SetOrderButtonHidden: not yet implemented");
						return null;
					});
	...
	public static void registerConfigNatives(final JassProgram jassProgramVisitor, final War3MapConfig mapConfig,
		...
		jassProgramVisitor.getJassNativeManager().createNative("IsPlayerAlly",
				(arguments, globalScope, triggerScope) -> {
					final CPlayerJass player = nullable(arguments, 0,
							ObjectJassValueVisitor.<CPlayerJass>getInstance());
					final CPlayerJass otherPlayer = nullable(arguments, 1,
							ObjectJassValueVisitor.<CPlayerJass>getInstance());
					if ((player == null) || (otherPlayer == null)) {
						return BooleanJassValue.FALSE;
					}
					return BooleanJassValue
							.of(player.hasAlliance(otherPlayer.getId(), CAllianceType.PASSIVE));
				});
		jassProgramVisitor.getJassNativeManager().createNative("IsPlayerEnemy",
				(arguments, globalScope, triggerScope) -> {
					final CPlayerJass player = nullable(arguments, 0,
							ObjectJassValueVisitor.<CPlayerJass>getInstance());
					final CPlayerJass otherPlayer = nullable(arguments, 1,
		...
		jassProgramVisitor.getJassNativeManager().createNative("IsPlayerType",
				(arguments, globalScope, triggerScope) -> {
					final CPlayerJass player = nullable(arguments, 0,
							ObjectJassValueVisitor.<CPlayerJass>getInstance());
					final CMapControl whichType = nullable(arguments, 1,
							ObjectJassValueVisitor.<CMapControl>getInstance());
					if (player == null || whichType == null) {
						return BooleanJassValue.FALSE;
					}
					return BooleanJassValue.of(player.getController() == whichType);
				});
		...
		jassProgramVisitor.getJassNativeManager().createNative("SetPlayerState",
				(arguments, globalScope, triggerScope) -> {
					final CPlayerJass player = nullable(arguments, 0,
							ObjectJassValueVisitor.<CPlayerJass>getInstance());
					final CPlayerState whichPlayerState = nullable(arguments, 1,
							ObjectJassValueVisitor.<CPlayerState>getInstance());
					final int value = arguments.get(2).visit(IntegerJassValueVisitor.getInstance());
					if ((player != null) && (whichPlayerState != null)) {
						player.setPlayerState(whichPlayerState, value);
					}
					return null;
				});
		jassProgramVisitor.getJassNativeManager().createNative("GetPlayerState",
				(arguments, globalScope, triggerScope) -> {
					...
					final CPlayerState whichPlayerState = nullable(arguments, 1,
							ObjectJassValueVisitor.<CPlayerState>getInstance());
					if ((player != null) && (whichPlayerState != null)) {
						return IntegerJassValue.of(player.getPlayerState(whichPlayerState));
					}
					return IntegerJassValue.ZERO;
				});
	...
	public static void registerTypingNatives(final JassProgram jassProgramVisitor, final HandleJassType raceType,
		...
		jassProgramVisitor.getJassNativeManager().createNative("ConvertRace",
				(arguments, globalScope, triggerScope) -> {
					final int i = arguments.get(0).visit(IntegerJassValueVisitor.getInstance());
					CRace race = WarsmashConstants.RACE_MANAGER != null ? WarsmashConstants.RACE_MANAGER.getRace(i) : null;
					if (race == null) {
		...
		jassProgramVisitor.getJassNativeManager().createNative("ConvertRacePref",
				(arguments, globalScope, triggerScope) -> {
					final int i = arguments.get(0).visit(IntegerJassValueVisitor.getInstance());
					CRacePreference racePreference = WarsmashConstants.RACE_MANAGER != null
							? WarsmashConstants.RACE_MANAGER.getRacePreference(i)
							: null;
